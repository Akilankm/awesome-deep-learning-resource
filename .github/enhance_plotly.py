...
def _cell(kind: str, source: str, key: str) -> dict:
    cell = {"cell_type": kind, "id": _id(key), "metadata": {}, "source": source.splitlines(keepends=True)}
    if kind == "code":
...
def append_lab(filename: str, title: str, explanation: str, code: str) -> None:
    ...
    key = filename + title
    nb["cells"].extend([_cell("markdown", markdown, "md-" + key), _cell("code", code, "code-" + key)])
...
def main() -> None:
    ensure_dependencies(); update_readme()
    append_lab("03_neuron_linear_algebra_architecture.ipynb", "Interactive neuron geometry: weights and bias reshape the affine surface", "Rotate the 3D surface and move the parameter slider. The surface is the pre-activation $z=w_1x_1+w_2x_2+b$: weights change orientation while bias translates it. Hover exposes the exact numerical pre-activation at any input coordinate.", NEURON)
    append_lab("04_activations_initialization_gradient_flow.ipynb", "Interactive activation + derivative explorer", "Use the dropdown to compare activation values and local derivatives. Backpropagation multiplies by this local slope, so near-zero regions make saturation and vanishing gradients directly inspectable.", ACTIVATIONS)
    append_lab("05_forward_logits_softmax.ipynb", "Interactive softmax confidence and temperature", "The logits stay fixed while temperature changes. Class ranking can remain unchanged while probability sharpness changes substantially, which matters for confidence interpretation, calibration and decision thresholds.", SOFTMAX)
    append_lab("07_optimizers_end_to_end_training.ipynb", "Interactive 3D loss landscape and optimizer trajectories", "Rotate the surface, hover exact loss values and compare learning rates. This deliberately low-dimensional landscape exposes how step size changes convergence, overshoot and oscillation.", OPTIMIZATION)
    append_lab("09_representation_learning_visualized.ipynb", "Interactive representation learning across epochs", "This uses the notebook's real `snaps` dictionary: the **same 1,500 MNIST samples** at multiple epochs. Play the animation or move the epoch slider, then hover one sample to track how learned hidden coordinates change.", REPRESENTATION)
    append_lab("10_test_evaluation_error_calibration.ipynb", "Interactive calibration and confidence inspection", "This uses the notebook's actual test-set `probs` and `y_test`. Hover the reliability curve to compare mean predicted confidence with empirical accuracy and see how many samples support each bin.", CALIBRATION)
    append_lab("12_production_monitoring_drift_retraining_business.ipynb", "Interactive production drift simulator", "The slider progressively darkens the notebook's real normalized MNIST test pixels. Labels do not change, but the input distribution does, making covariate shift tangible before connecting it to monitoring, delayed-label performance and retraining governance.", DRIFT)
